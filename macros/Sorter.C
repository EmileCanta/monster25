...
{
    Double_t time_aligned = time_offset_plastic[index-3] + 5.;
...
void Sorter::SetTreesAndBranches(const char* OutputFileName)
{
    ...
    output_tree_single->Branch("MrBig_ESingle", &fMrBig_ESingle);
...
void Sorter::FillSingleBranches()
{
    ...
    for(fEntry = 0; fEntry < fEntries; fEntry++)
    {
        ...
        if(raw_det_nbr == 1)
        {
            fPlastic_tSingle.push_back((raw_time / 1.e9));
        }

        if(raw_det_nbr == 2)
        {
            fMrBig_ESingle.push_back(MrBig_alignement(raw_energy1));
        }
...
void Sorter::BackwardCoinc(Double_t coincwindow)
{
    std::cout << "Backward coincs" << std::endl;
	...
	for(fEntry = fEntries; fEntry > 0; fEntry--)
	{
        ...
        {   
            ...
            while(TMath::Abs(raw_time - fPlastic_Time) < coincwindow)
            {
	      		if(raw_det_nbr == 1) break;

                raw_data_tree->GetEntry(fEntry - eventafter++);

                if(fEntry - eventafter < 0) break;
                
                if(TMath::Abs(raw_time - fPlastic_Time) > coincwindow) continue;
                if(TMath::Abs(raw_time - fPlastic_Time) < coincwindow)
                {   
...
void Sorter::ForwardCoinc(Double_t coincwindow)
{
    std::cout << "Forward coincs" << std::endl;
